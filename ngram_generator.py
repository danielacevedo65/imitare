...
class ngram_generator:
    ...
    def _choose_word_by_tag(self, tag, model, context=()):
        if len(context) == 0:
            ...
            context = (model._lpad + tuple(context))[1 - self._n:]
            if context in model:
...
if __name__ == '__main__':
    ...
    model = ngram_generator(stanford_postagger().tag(stanford_tokenizer().tokenize(text)), int(sys.argv[2]))
